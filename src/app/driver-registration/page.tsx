...
const step3Schema = z.object({
    drivingLicenseNumber: z.string().min(5, "License number is required."),
    licenseType: z.string().min(2, "License type is required."),
    licenseIssuingAuthority: z.string().min(2, "Issuing authority is required."),
    licenseIssueDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: 'Invalid issue date' }),
    licenseExpiryDate: z.string().refine((val) => !isNaN(Date.parse(val)), { message: 'Invalid expiry date' }),
    drivingExperience: z.coerce.number().min(0, "Experience cannot be negative."),
    badgeNumber: z.string().optional(),
});

const step4Schema = z.object({
    medicalFitnessCertificate: z.enum(["Yes", "No"]).optional(),
    medicalConditions: z.string().optional(),
    visionTestStatus: z.string().optional(),
});
...
const step5Schema = z.object({
    ...
    licenseCopyUrl: z.string().url("Please enter a valid URL.").optional().or(z.literal('')),
    medicalCertificateUrl: z.string().url("Please enter a valid URL.").optional().or(z.literal('')),
    policeVerificationUrl: z.string().url("Please enter a valid URL.").optional().or(z.literal('')),
    experienceCertificateUrl: z.string().url("Please enter a valid URL.").optional().or(z.literal('')),
});
...
export default function DriverRegistrationPage() {
  ...
  const steps = [
    ...
    { id: 'Step 3', name: 'License Details', fields: ['drivingLicenseNumber', 'licenseType', 'licenseIssuingAuthority', 'licenseIssueDate', 'licenseExpiryDate', 'drivingExperience', 'badgeNumber'] },
    { id: 'Step 4', name: 'Health Information', fields: ['medicalFitnessCertificate', 'medicalConditions', 'visionTestStatus'] },
    { id: 'Step 5', name: 'Documents Links', fields: ['photographUrl', 'aadharCardUrl', 'licenseCopyUrl', 'medicalCertificateUrl', 'policeVerificationUrl', 'experienceCertificateUrl'] },
  ];
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <FormField control={form.control} name="drivingLicenseNumber" render={({ field }) => (
                      ...
                      <FormField control={form.control} name="licenseType" render={({ field }) => (
                        <FormItem><FormLabel>License Type *</FormLabel><FormControl><Input placeholder="e.g., LMV, HMV, PSV" {...field} /></FormControl><FormMessage /></FormItem>
                     )} />
                      <FormField control={form.control} name="licenseIssuingAuthority" render={({ field }) => (
                        <FormItem><FormLabel>License Issuing Authority *</FormLabel><FormControl><Input placeholder="e.g., RTO Delhi" {...field} /></FormControl><FormMessage /></FormItem>
                     )} />
                      <FormField control={form.control} name="drivingExperience" render={({ field }) => (
                        <FormItem><FormLabel>Years of Driving Experience *</FormLabel><FormControl><Input type="number" placeholder="e.g., 5" {...field} /></FormControl><FormMessage /></FormItem>
                     )} />
                     <FormField control={form.control} name="licenseIssueDate" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="licenseExpiryDate" render={({ field }) => (
                      ...
                      <FormField control={form.control} name="badgeNumber" render={({ field }) => (
                        <FormItem className="md:col-span-2"><FormLabel>Badge Number (if applicable)</FormLabel><FormControl><Input placeholder="Commercial vehicle badge number" {...field} /></FormControl><FormMessage /></FormItem>
                     )} />
                  </div>
                </div>
                ...
                <div className={currentStep === 3 ? 'block' : 'hidden'}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <FormField control={form.control} name="medicalFitnessCertificate" render={({ field }) => (
                        <FormItem>
                            <FormLabel>Medical Fitness Certificate Available?</FormLabel>
                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                         ...
                         <FormField control={form.control} name="visionTestStatus" render={({ field }) => (
                            <FormItem><FormLabel>Vision Test Status</FormLabel><FormControl><Input placeholder="e.g., Passed, Due" {...field} /></FormControl><FormMessage /></FormItem>
                         )} />
                         <FormField control={form.control} name="medicalConditions" render={({ field }) => (
                            <FormItem className="md:col-span-2"><FormLabel>Any Medical Conditions?</FormLabel><FormControl><Textarea placeholder="List any known medical conditions like Asthma, Epilepsy, etc." {...field} /></FormControl><FormMessage /></FormItem>
                         )} />
                    </div>
                </div>
                ...
                <div className={currentStep === 4 ? 'block' : 'hidden'}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     ...
                     <FormField control={form.control} name="aadharCardUrl" render={({ field }) => (
                        <FormItem><FormLabel>Aadhar Card / ID Proof URL</FormLabel><FormControl><Input placeholder="https://your-link.com/aadhar.pdf" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                     <FormField control={form.control} name="licenseCopyUrl" render={({ field }) => (
                        <FormItem><FormLabel>Driving License Copy URL</FormLabel><FormControl><Input placeholder="https://your-link.com/license.pdf" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="medicalCertificateUrl" render={({ field }) => (
                        <FormItem><FormLabel>Medical Fitness Certificate URL</FormLabel><FormControl><Input placeholder="https://your-link.com/medical.pdf" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="policeVerificationUrl" render={({ field }) => (
                        <FormItem><FormLabel>Police Verification Certificate URL</FormLabel><FormControl><Input placeholder="https://your-link.com/police.pdf" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                    <FormField control={form.control} name="experienceCertificateUrl" render={({ field }) => (
                        <FormItem><FormLabel>Experience Certificate URL</FormLabel><FormControl><Input placeholder="https://your-link.com/experience.pdf" {...field} /></FormControl><FormMessage /></FormItem>
                    )} />
                  </div>
                </div>
